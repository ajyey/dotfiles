...
install_packages() {
  log "Installing Debian packages with apt"
  ...
  if [ "${#missing[@]}" -gt 0 ]; then
    warn "these apt packages were unavailable and were skipped: ${missing[*]}"
    warn "install skipped tools manually if this Debian release does not package them."
  fi
